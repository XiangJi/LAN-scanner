...
class SubmitAction implements ActionListener {
	...
	public void actionPerformed(ActionEvent a) {
		...
		try {
			TCPThread.hostAddress = InetAddress.getByName(ThreadScan.hostname
					.getText());
		} catch (UnknownHostException e) {
			ThreadScan.DLGINFO
					.setText("            错误的IP地址/域名或地址不可达!            ");
			ThreadScan.DLGError.setVisible(true);
			ThreadScan.status.setText("Scanning status: Not started");
			return;
		}
		...
		ThreadScan.Result
				.append("Scanning " + ThreadScan.hostname.getText()
						+ " Number of threads "
