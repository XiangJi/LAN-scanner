...
class TCPThread extends Thread {
	...
	public void run() {
		int i;
		if (flag == 1) {
			if (SubmitAction.flag1 == 0) {
				flag = 0;
				SubmitAction.flag1 = 1;
			}
		}
		Socket theTCPsocket;
		for (i = MIN_port + threadnum; i <= MAX_port; i += Integer.parseInt(ThreadScan.maxThread.getText())) {
			ThreadScan.status.setText("Scanning status: scanning " + i
					+ " Port");
			// System.out.print(i+" ");
			try {
		...
		if (i > MAX_port
				&& threadnum == Integer
						.parseInt(ThreadScan.maxThread.getText()) - 1
				&& flag == 0) {
			flag = 1;
			ThreadScan.Result.append("\n" + "Scanning completed...");
